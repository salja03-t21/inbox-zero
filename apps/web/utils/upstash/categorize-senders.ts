...
/**
 * Publishes sender categorization tasks to queue in batches
 * Splits large arrays of senders into chunks of BATCH_SIZE to prevent overwhelming the system
 * Uses the queue abstraction layer which supports both Inngest and QStash
 */
export async function publishToAiCategorizeSendersQueue(
  ...
  await Promise.all(
    chunks.map((senderChunk) =>
      enqueueJob({
        name: "inbox-zero/categorize.senders-batch",
        data: {
          emailAccountId: body.emailAccountId,
          senders: senderChunk,
