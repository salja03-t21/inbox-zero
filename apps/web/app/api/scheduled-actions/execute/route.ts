...
export const POST = withError(async (request: NextRequest) => {
  ...
  if (internalKey === env.INTERNAL_API_KEY) {
    logger.info("Processing scheduled action via internal API");
    return handleRequest(request);
  }

  // Otherwise, verify QStash signature
  logger.info("Processing scheduled action via QStash");
...
async function handleRequest(request: NextRequest) {
  try {
    ...
    logger.info("Received scheduled action execution request", {
    ...
    logger.error("Scheduled action execution failed", { error });
